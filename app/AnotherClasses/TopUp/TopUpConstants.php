...
class TopUpConstants
{
    ...
    const MAKE_PAYMENT_BODY_TEMPLATE = '<auth><payment><transaction-number>{transaction_number}</transaction-number><from><ccy>{currency}</ccy></from><to><amount>{amount}</amount><ccy>{currency}</ccy><service-id>99</service-id><account-number>{account_number}</account-number></to></payment></auth>';
}